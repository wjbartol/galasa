...
public class KarafTest {
    ...
    /**
     * Setup a golden path Maven project with dependencies to exercise logic in the
     * goa;
     * 
    ...
    public void setup() throws Exception {
        ...
        DefaultArtifact depArtifactUnresolved = new DefaultArtifact("group2", "unresolved1", "1.2.5", "compile", "xml",
                "xml", null);
        DefaultArtifact depArtifactProvided = new DefaultArtifact("group2", "provided1", "1.2.5", "provided", "obr",
                "obr", null);
    ...
    public void testGoldenPath() throws Exception {
        ...
        Assert.assertEquals("Should contain only 1 features", 1,
                StringUtils.countMatches(featureXml,
                        "<features xmlns=\"http://karaf.apache.org/xmlns/features/v1.3.0\" name=\"" + PROJECT_ID + "-"
                                + PROJECT_VERSION + "\">"));
        Assert.assertEquals("Should contain 2 <feature name=", 2,
                StringUtils.countMatches(featureXml, "<feature name="));
        Assert.assertEquals("Should contain 2 <feature>", 2,
                StringUtils.countMatches(featureXml, "<feature>" + FEATURE_ID + "</feature>"));
        Assert.assertEquals("Should contain 2 <bundle>", 2,
                StringUtils.countMatches(featureXml, "<bundle>" + BUNDLE_URI + "</bundle>"));
    }
    ...
    public void testNewOutputPath() throws Exception {
        ...
        String featureXml = FileUtils.readFileToString(featureXmlFile);
        Assert.assertEquals("Does not contain <features ", 1,
                StringUtils.countMatches(featureXml,
                        "<features xmlns=\"http://karaf.apache.org/xmlns/features/v1.3.0\" name=\"" + PROJECT_ID + "-"
                                + PROJECT_VERSION + "\">"));
    }
    ...
    public void testNoFeature() throws Exception {
        ...
        Assert.assertEquals("Should contain 2 <feature name=", 2,
                StringUtils.countMatches(featureXml, "<feature name="));
        Assert.assertTrue("Should default feature name",
                StringUtils.contains(featureXml, "<feature name=\"feature1\""));
